(function($){
  $(document).ready(function(){
	...
	$('input#ship_to_use_billing').live('click', function(){
        $('#shipping .inner input, #shipping .inner select, #shipping .inner label, #shipping .inner .req, #shipping .inner p.field').hide();
        $('#shipping .inner input, #shipping .inner select').attr('disabled', 'disabled');
		$('#order_use_bill_address').val('1');	
	})

	$('input#ship_to_use_other').live('click', function(){
        $('#shipping .inner input, #shipping .inner select, #shipping .inner label, #shipping .inner .req, #shipping .inner p.field').show();
        $('#shipping .inner input, #shipping .inner select').removeAttr('disabled', 'disabled');
		$('#order_use_bill_address').val('0');
        //only want to enable relevant field
        if(get_states('s')){
          $('span#sstate input').hide().attr('disabled', 'disabled');
        }else{
