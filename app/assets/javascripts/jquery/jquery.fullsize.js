				...
				if($(this).attr("longdesc")) {
					...
					$(icon).click(function(){
						...
						$(full_img).load(function(){
							...
							$('body').click(function() {
								//if ($(event.target).parents('.fullsize-icon').length == 0) {
									closeFullsize(full_img, full_wrap, org_image, $('.fullsize-close'));	
								//}
							});
									
						}).attr("src",$("#main-image img").attr('longdesc').replace('/product/', '/original/').replace('/large/', '/original/')).addClass("fullsize-image");
