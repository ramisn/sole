module Spree
  ...
  module ProductFilters
    ...
    Spree::Product.scope :new_price_range_any,
       lambda {|*opts|
         conds = opts.map {|o| Spree::ProductFilters.new_price_filter[:conds][o]}.reject {|c| c.nil?}
         Spree::Product.scoped(:joins => :master).conditions_any(conds)
       }

 
     def ProductFilters.new_price_filter
       conds = [ [ "Under $10",    "price             <= 10" ],
                 [ "$10 - $15",    "price between 10 and 15" ],
                 [ "$15 - $18",    "price between 15 and 18" ],
                 [ "$18 - $20",    "price between 18 and 20" ],
                 [ "$30 - $60",    "price between 30 and 60" ],
                 [ "$20 or over",  "price             >= 20" ] ]
       { :name   => "Price Range",
         :scope  => :new_price_range_any,
         :conds  => Hash[*conds.flatten],
         :labels => conds.map {|k,v| [k,k]}
       }
     end
